...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: hs } = await supabase().from("handles").select("phone, handle, bio, students(name)").in("phone", others.length ? others : ["_"]);
    const info = new Map<string, { handle: string; name: string; bio: string | null }>();
    for (const h of hs ?? []) {
      const s = Array.isArray(h.students) ? h.students[0] : h.students;
      info.set(h.phone as string, { handle: h.handle as string, name: (s as { name: string } | null)?.name ?? "", bio: (h.bio as string | null) ?? null });
    }
    ...
    // Incoming requests carry the sender's bio — the one place non-friends may flaunt it.
    const incoming = rows.filter((r) => r.status === "pending" && r.addressee_phone === me).map((r) => ({ id: r.id, ...pick(r.requester_phone), bio: info.get(r.requester_phone)?.bio ?? null }));
